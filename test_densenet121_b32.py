...
class DenseNet121_ViTB32_Hybrid(nn.Module):
    def __init__(self, num_classes=2):
        super(DenseNet121_ViTB32_Hybrid, self).__init__()

        self.cnn = models.densenet121(pretrained=True)
        self.cnn.classifier = nn.Identity()

        self.vit = ViTForImageClassification.from_pretrained('google/vit-base-patch32-224')
    ...
    def forward(self, x):
        ...
        vit_features = self.vit(pixel_values=x).logits
...
def train_client(model, train_loader, optimizer, criterion, epochs, scheduler):
    ...
    layers_to_unfreeze = [
        model.cnn.features.denseblock4,
        model.cnn.features.denseblock3,
        model.cnn.features.denseblock2,
...
def federated_learning(num_rounds, local_epochs, patience=5):
    global_model = DenseNet121_ViTB32_Hybrid(num_classes=2).to(device)
    ...
    for round in range(num_rounds):
        ...
        for client_loader in client_loaders:
            client_model = DenseNet121_ViTB32_Hybrid(num_classes=2).to(device)
